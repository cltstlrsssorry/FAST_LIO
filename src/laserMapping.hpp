...
void publish_raw_cloud(rclcpp::Publisher<sensor_msgs::msg::PointCloud2>::SharedPtr pubLaserCloudFull_ptr_, PointCloudXYZI::Ptr cloud_ptr_)
{
    sensor_msgs::msg::PointCloud2 laserCloudmsg;
    pcl::toROSMsg(*cloud_ptr_, laserCloudmsg);
    // laserCloudmsg.header.stamp = ros::Time().fromSec(lidar_end_time);
    laserCloudmsg.header.stamp = get_ros_time(lidar_end_time);
    laserCloudmsg.header.frame_id = "camera_init";
    pubLaserCloudFull_ptr_->publish(laserCloudmsg);
}
...
public:
    ...
    {
        RCLCPP_INFO(this->get_logger(), "----LaserMappingNode----");
...
private:
    ...
    void timer_callback()
    {
        ...
        counters_raw_map++;
        if (counters_raw_map % 10 == 0 && filter_map_enable)
        {
