...
void FreeSeg(float *fPoints,int *pLabel,int pointNum);
...
class PCSeg
{
    ...
    PCSeg();
    // functions
    void DoSeg(int *pLabel, float* fPoints1, int pointNum);
    int GetMainVectors(float*fPoints, int* pLabel, int pointNum);
    void EncodeFeatures(float *pFeas);
